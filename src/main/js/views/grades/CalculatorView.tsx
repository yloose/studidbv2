...
const CalculatorView = () => {
    ...
    const [modules, setModules] = useState([
        { id: 1, name: 'Mathematics', grade: 1.3, credits: 6, semester: 'WS 22/23' },
        { id: 2, name: 'Programming', grade: 1.7, credits: 8, semester: 'WS 22/23' },
        { id: 3, name: 'Data Structures', grade: 2.3, credits: 6, semester: 'SS 23' },
        { id: 4, name: 'Algorithms', grade: 1.0, credits: 8, semester: 'SS 23' }
    ]);
    ...
    const calculateSimpleAverage = () => {
        ...
        const sum = modules.reduce((total, module) => total + module.grade, 0);
    ...
    const calculateWeightedAverage = () => {
        if (modules.length === 0) return 0;
        const totalWeightedGrade = modules.reduce((total, module) =>
            total + ((module.grade) * module.credits), 0);
        const totalCredits = modules.reduce((total, module) =>
            total + module.credits, 0);
    ...
    const calculateHypotheticalAverage = () => {
        if (!newModule.grade || !newModule.credits) return calculateWeightedAverage();

        const hypotheticalModules = [...modules, {
            ...newModule,
            grade: parseFloat(newModule.grade),
            credits: parseFloat(newModule.credits),
            semester: newModule.semester || currentSemester
        }];
    ...
    const getSortedModules = () => {
        ...
        switch (sortBy) {
            case 'best':
                return sortedModules.sort((a, b) => a.grade - b.grade);
            case 'worst':
                return sortedModules.sort((a, b) => b.grade - a.grade);
            case 'latest':
                // Simple sort for the sample data
                return sortedModules.sort((a, b) => a.semester < b.semester ? 1 : -1);
            case 'oldest':
                return sortedModules.sort((a, b) => a.semester > b.semester ? 1 : -1);
            case 'highest-ects':
                return sortedModules.sort((a, b) => b.credits - a.credits);
            default:
                                ...
                                {getSortedModules().map((module, index) => (
                                    <div
                                        ...
                                        className={`flex justify-between items-center py-3 ${
                                            index !== modules.length - 1 ? 'border-b border-gray-200' : ''
                                        }`}
                                    >
                                        <div>
                                            <h4 className="font-medium text-gray-800">{module.name}</h4>
                                            <p className="text-sm text-gray-500">{module.semester}</p>
                                        </div>
                                        <div className="flex items-center gap-3">
                                            <span className="bg-blue-50 text-blue-600 px-2 py-1 rounded text-sm">{module.credits} ECTS</span>
                                            <span className={`font-bold text-lg ${
                                                module.grade <= 1.5 ? 'text-green-600' :
                                                    module.grade <= 2.5 ? 'text-blue-600' :
                                                        module.grade <= 3.5 ? 'text-orange-500' : 'text-red-500'
                                            }`}>
